...
const AdminSeasons = () => {
  ...
  const handleCreate = async () => {
    ...
    if (seasonType === "blood" && selectedScripts.length > 0) {
      await supabase
        ...
        .insert(selectedScripts.map((sid) => ({ season_id: data.id, script_id: sid })) as any);
    }
  ...
  const toggleGameInSeason = async (seasonId: string, gameId: string, isCurrently: boolean) => {
    if (isCurrently) {
      ...
      if (count && count > 0) {
        return notify(
          "error",
          `Não é possível remover: existem ${count} partida(s) registrada(s) para este jogo nesta season.`,
        );
      }
  ...
  const toggleScriptInSeason = async (seasonId: string, scriptId: string, isCurrently: boolean) => {
    if (isCurrently) {
      ...
      if (count && count > 0) {
        return notify(
          "error",
          `Não é possível remover: existem ${count} partida(s) registrada(s) para este script nesta season.`,
        );
      }
  ...
  const renderPrizeFields = (
    isBlood: boolean,
    values: { p1: string; p2: string; p3: string; p4: string; p5: string },
      ...
      return (
        <div>
          ...
          <div className="flex gap-4 mt-1 text-sm flex-wrap">
            {s.prize_1st > 0 && (
              <span>
                🥇 1º-3º: <strong className="text-gold">R$ {s.prize_1st} cada</strong>
              </span>
            )}
            {s.prize_4th_6th > 0 && (
              <span>
                🥈 4º-6º: <strong>R$ {s.prize_4th_6th} cada</strong>
              </span>
            )}
            {s.prize_7th_10th > 0 && (
              <span>
                🥉 7º-10º: <strong>R$ {s.prize_7th_10th} cada</strong>
              </span>
            )}
          </div>
    ...
    return (
      <div>
        ...
        <div className="flex gap-4 mt-1 text-sm flex-wrap">
          {s.prize_1st > 0 && (
            <span>
              🥇 1º: <strong className="text-gold">R$ {s.prize_1st}</strong>
            </span>
          )}
          {s.prize_2nd > 0 && (
            <span>
              🥈 2º: <strong>R$ {s.prize_2nd}</strong>
            </span>
          )}
          {s.prize_3rd > 0 && (
            <span>
              🥉 3º: <strong>R$ {s.prize_3rd}</strong>
            </span>
          )}
        </div>
        <p className="text-xs text-muted-foreground mt-1">Total: R$ {total}</p>
  ...
  return (
    <div className="space-y-6">
      <Card className="bg-card border-border">
        <CardHeader>
          <CardTitle>Nova Season</CardTitle>
        </CardHeader>
        <CardContent className="space-y-4">
          ...
          <div className="space-y-2">
            ...
            <Input
              ...
              placeholder="Descrição da season"
            />
          </div>
                    ...
                    <Checkbox
                      ...
                      onCheckedChange={() =>
                        setSelectedScripts((prev) =>
                          prev.includes(bs.id) ? prev.filter((id) => id !== bs.id) : [...prev, bs.id],
                        )
                      }
                    />
          ...
          <Button variant="gold" onClick={handleCreate}>
            <Plus className="h-4 w-4 mr-1" /> Criar Season
          </Button>
        </CardContent>
      </Card>
                            ...
                            {bloodScripts.map((bs) => {
                              ...
                              return (
                                ...
                                >
                                  <Checkbox
                                    checked={isLinked}
                                    onCheckedChange={() => toggleScriptInSeason(s.id, bs.id, isLinked)}
                                  />
                                  <span className="text-sm">{bs.name}</span>
                                </label>
                              );
          ...
          <div className="space-y-4 max-h-[70vh] overflow-y-auto pr-2">
            ...
            <div className="space-y-2">
              ...
              {renderPrizeFields(
                ...
                {
                  p1: editForm.prize_1st,
                  p2: editForm.prize_2nd,
                  p3: editForm.prize_3rd,
                  p4: editForm.prize_4th_6th,
                  p5: editForm.prize_7th_10th,
                },
                (field, val) => {
                    ...
                    <div className="grid gap-2 sm:grid-cols-2">
                      {games.map((g) => {
                        const isLinked = editingSeason
                          ? (seasonGamesMap[editingSeason.id] || []).includes(g.id)
                          : false;
                        return (
                          <label
                            key={g.id}
                          ...
                          >
                            <Checkbox
                              checked={isLinked}
                              onCheckedChange={() =>
                                editingSeason && toggleGameInSeason(editingSeason.id, g.id, isLinked)
                              }
                            />
                            <span className="text-sm">{g.name}</span>
                          </label>
