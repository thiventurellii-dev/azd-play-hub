...
const EditBloodScriptDialog = ({ open, onOpenChange, script, onSaved, showCharacters = false }: Props) => {
  ...
  useEffect(() => {
    ...
    setNewCondition('');
    if (showCharacters) fetchCharacters();
  ...
  const handleCreateChar = async () => {
    if (!script || !charName || !charNameEn) return toast.error('Preencha nome (PT) e nome (EN)');
  ...
  const handleAddExistingChar = async () => {
    if (!script || !addExistingCharId) return toast.error('Selecione um personagem');
    const { error } = await supabase.from('blood_characters').update({ script_id: script.id }).eq('id', addExistingCharId);
    if (error) return toast.error(error.message);
    toast.success('Personagem adicionado ao script!');
  ...
  return (
    <>
      <EntitySheet
        ...
        title="Editar Script"
        description="Atualize os dados do script de Blood on the Clocktower."
      >
      ...
      {/* Edit Character sub-dialog (stays as Dialog since it's a nested overlay) */}
