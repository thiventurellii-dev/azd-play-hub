...
interface ScoringSchema {
  categories: { key: string; label: string; type?: string; subcategories?: SchemaField[] }[];
}
...
const PLATFORMS = ["Presencial", "Tabletop Simulator", "BoardGame Arena", "Foundry", "Outro Online"];

const NewBoardgameFlow = ({
  onComplete,
  prefilledGameId,
  prefilledPlayers,
  prefilledDate,
  ...
  useEffect(() => {
    const fetchBase = async () => {
      ...
      const recent = ordered
        .slice(0, 4)
        .map((gid) => allGames.find((g) => g.id === gid))
  ...
  const setSimpleScore = (i: number, raw: string) => {
    if (raw === "") {
      updateEntry(i, { total_score: null });
      return;
    }
  ...
  const importLastGroup = async () => {
    if (!gameId) {
      notify("error", "Selecione um jogo primeiro");
      return;
    }
    const { data: lastMatch } = await supabase
      ...
      .maybeSingle();
    if (!lastMatch) {
      notify("error", "Nenhuma partida anterior deste jogo");
      return;
    }
    const { data: results } = await supabase
      .from("match_results")
      .select("player_id, seat_position")
      .eq("match_id", (lastMatch as any).id)
      .order("seat_position", { ascending: true });
    if (!results || results.length === 0) {
      notify("error", "Sem jogadores na última partida");
      return;
    }
    setEntries(
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      if (!effectiveSeasonId) {
        notify("error", "Nenhuma season disponível");
        setSaving(false);
        return;
      }
  ...
  const sectionDoneClass = (done: boolean) => (done ? "border-green-500/20" : "border-gold/25");

  const StatusBubble = ({ done, n }: { done: boolean; n: number }) =>
    done ? (
  ...
  return (
    <div className="space-y-4 pb-24">
        ...
        {gameId && seasons.find((s) => s.status === "active") && activeSeasonGameIds.includes(gameId) && (
          <label className="flex items-center justify-between gap-3 rounded-lg border border-gold/30 bg-gold/5 px-3 py-2 cursor-pointer">
            <div className="flex items-center gap-2">
              ...
              />
              <span className="text-sm font-medium">Vale para {seasons.find((s) => s.status === "active")?.name}</span>
            </div>
      ...
      <div
        className={`rounded-xl border ${sectionDoneClass(filledCount > 0 && scoredCount === filledCount && playerCountValid)} bg-card/60 p-5 space-y-4`}
      >
        <div className="flex items-center gap-3">
          ...
          {filledCount > 0 && (
            <span className="ml-auto text-xs text-gold/80">
              {scoredCount} de {filledCount} com pontuação
            </span>
          )}
        </div>
        ...
        <div className="flex flex-wrap gap-2">
          ...
          {friendIds.length > 0 && (
            <Popover
              ...
              onOpenChange={(o) => {
                setFriendsOpen(o);
                if (!o) setSelectedFriendsToAdd(new Set());
              }}
            >
                        ...
                        .map((p) => {
                          ...
                          return (
                            ...
                            >
                              <Checkbox
                                checked={checked}
                                className="mr-2 data-[state=checked]:bg-gold data-[state=checked]:border-gold"
                              />
          ...
          {selectedCommunityId && communityMemberIds.length > 0 && (
            <Popover
              ...
              onOpenChange={(o) => {
                setCommunityOpen(o);
                if (!o) setSelectedCommunityToAdd(new Set());
              }}
            >
              <PopoverTrigger asChild>
                <button className="inline-flex items-center gap-1.5 rounded-full border border-[#7ee787]/30 bg-[#7ee787]/10 px-3 py-1 text-xs text-[#7ee787] hover:bg-[#7ee787]/20">
              ...
              <PopoverContent className="w-[280px] p-0" align="start">
                ...
                <div className="flex items-center justify-between gap-2 border-t border-border p-2">
                  <span className="text-[11px] text-muted-foreground">
                    {selectedCommunityToAdd.size} selecionado(s)
                  </span>
                  <Button
                    size="sm"
                    variant="gold"
              ...
              <div key={i} className="space-y-2">
                <div className="grid grid-cols-[40px_1fr_auto_auto_auto_32px] items-center gap-3 rounded-lg border border-border/40 bg-background/40 px-2 py-2">
                  ...
                  {gameFactions.length > 0 && (
                    <Select
                      value={e.faction || "none"}
                      onValueChange={(v) => updateEntry(i, { faction: v === "none" ? "" : v })}
                    >
                  ...
                  {hasSchema ? (
                    <button
                      onClick={() => {
                        ...
                        updateEntry(i, { scoring_open: !e.scoring_open });
                      }}
                    ...
                    >
                      ...
                      <ChevronDown
                        className={`h-3 w-3 text-muted-foreground transition-transform ${e.scoring_open ? "rotate-180" : ""}`}
                      />
                    </button>
                  ) : (
                    <Input
                ...
                {hasSchema && e.scoring_open && (
                  <div className="rounded-lg border border-[#c4a8ff]/30 bg-[#c4a8ff]/5 p-3 space-y-2">
                    <div className="flex items-center justify-between">
                      <span className="text-[11px] uppercase tracking-wide text-[#c4a8ff] font-semibold">
                        Detalhamento da pontuação
                      </span>
                      <button
                        onClick={() => updateEntry(i, { scoring_open: false })}
                        className="text-[11px] text-[#c4a8ff] hover:underline flex items-center gap-1"
                      >
        ...
        {!playerCountValid && filledCount > 0 && <p className="text-xs text-destructive">{progress.msg}</p>}
      </div>
          ...
          <div className="px-5 pb-5 space-y-3">
            <label className="flex items-center gap-2 rounded-md border border-dashed border-border px-4 py-3 cursor-pointer hover:bg-secondary/40">
              ...
              <span className="text-sm text-muted-foreground truncate flex-1">
                {imageFile ? imageFile.name : "Anexar foto da partida"}
              </span>
              <input
                type="file"
                accept="image/*"
...
const PodiumCell = ({ pos, hasScore }: { pos: number | null; hasScore: boolean }) => {
  ...
  if (pos === 1) {
    ...
    bg = "bg-[#c0c0c0]/15 text-[#d4d4d4] border-[#c0c0c0]/40";
  } else if (pos === 3) {
    bg = "bg-[#b87333]/15 text-[#d49060] border-[#b87333]/40";
  }
  return (
    <div
      className={`w-[60px] h-9 rounded border flex items-center justify-center gap-1 text-xs font-bold transition-colors ${bg}`}
    >
      {icon}
      {pos}º
    </div>
