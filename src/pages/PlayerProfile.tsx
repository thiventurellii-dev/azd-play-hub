...
const PlayerProfile = () => {
  ...
  useEffect(() => {
    const fetchProfile = async () => {
      const { data: prof } = await supabase.from('profiles').select('*').eq('nickname', nickname as string).maybeSingle();
      ...
      // Match results
      const { data: results } = await supabase.from('match_results').select('match_id, position, score').eq('player_id', prof.id);
      if (!results || results.length === 0) { setLoading(false); return; }

      const matchIds = [...new Set(results.map(r => r.match_id))];
      const { data: matches } = await supabase.from('matches').select('id, game_id, played_at').in('id', matchIds).order('played_at', { ascending: false });
      ...
      setGamePerformance(
        Object.entries(perfMap).map(([gid, s]) => ({
          game_id: gid,
      ...
      const { data: allResults } = await supabase.from('match_results').select('match_id, player_id, position').in('match_id', matchIds);
          ...
          <CardContent className="pt-6 flex flex-col sm:flex-row items-center gap-6">
            ...
            <div className="flex-1 text-center sm:text-left">
              ...
              <div className="flex items-center gap-2 justify-center sm:justify-start mt-2">
                <Badge variant={role === 'admin' ? 'default' : 'secondary'}>{role === 'admin' ? 'Admin' : 'Player'}</Badge>
                <span className="text-xs text-muted-foreground">Desde {new Date(profile.created_at).toLocaleDateString('pt-BR')}</span>
              </div>
            </div>
      ...
      {opponents.length > 0 && (
        <Card className="bg-card border-border">
          <CardContent className="pt-6">
            <h2 className="text-lg font-semibold mb-4">Principais Adversários</h2>
