...
const Seasons = () => {
  ...
  useEffect(() => {
    ...
    supabase
      ...
      .select("id, name")
      .order("name")
      .then(({ data }) => setAllScripts((data || []) as any[]));
  }, []);
    ...
    <div className="container py-8 space-y-6">
          ...
          <Dialog open={howOpen} onOpenChange={setHowOpen}>
            ...
            <DialogContent>
              ...
              <div className="space-y-3 text-sm text-muted-foreground">
                <p>
                  Cada Season é uma temporada oficial com início, fim e premiação. Você ganha MMR a cada partida
                  ranqueada e disputa o pódio até a data final.
                </p>
                <ul className="space-y-1.5 list-disc list-inside">
                  <li>
                    <strong className="text-foreground">Boardgame:</strong> ranking por MMR (ELO).
                  </li>
                  <li>
                    <strong className="text-foreground">Blood on the Clocktower:</strong> ranking por pontos (vitórias
                    do Mal valem 2, do Bem 1).
                  </li>
                </ul>
              </div>
            </DialogContent>
          ...
          <div className="space-y-6">
            ...
            <div>
              ...
              <p className="text-xs text-muted-foreground mb-4">
                Participe das temporadas em andamento e dispute o ranking!
              </p>
              {openSeasons.length === 0 ? (
                <Card className="bg-card border-border">
                  <CardContent className="py-12 text-center text-muted-foreground">Nenhuma season aberta.</CardContent>
                </Card>
              ) : (
                ...
                <button className="w-full flex items-center justify-between px-4 py-3 rounded-lg border border-border bg-card hover:border-gold/30 transition-colors">
                  <div className="text-left">
                    ...
                    <p className="text-xs text-muted-foreground">
                      {finishedSeasons.length} temporadas anteriores · clique para {closedOpen ? "ocultar" : "ver"}
                    </p>
                  </div>
                  <ChevronDown
                    className={`h-5 w-5 text-muted-foreground transition-transform ${closedOpen ? "rotate-180" : ""}`}
                  />
                </button>
              </CollapsibleTrigger>
                ...
                {finishedSeasons.length === 0 ? (
                  <Card className="bg-card border-border">
                    <CardContent className="py-8 text-center text-muted-foreground text-sm">
                      Nenhuma season encerrada ainda.
                    </CardContent>
                  </Card>
                ) : (
                  finishedSeasons.map((s) => <SeasonRowFinished key={s.id} season={s} linkedNames={linkedNames(s)} />)
                )}
              </CollapsibleContent>
            </Collapsible>
          </div>
      ...
      <Dialog open={dialogOpen} onOpenChange={setDialogOpen}>
        <DialogContent className="max-h-[90vh] overflow-y-auto">
          <DialogHeader>
            <DialogTitle>{editId ? "Editar Season" : "Criar Season"}</DialogTitle>
          </DialogHeader>
          <div className="space-y-4">
            ...
            <div className="space-y-2">
              ...
              <Input
                ...
                placeholder="https://... (opcional — usa imagem do jogo se vazio)"
              />
            </div>
            <div className="space-y-2">
              <Label>Tipo</Label>
            ...
            <div className="grid gap-4 grid-cols-2">
              <div className="space-y-2">
                <Label>Início *</Label>
                <DatePickerField value={formStart} onChange={setFormStart} placeholder="Data de início" />
              </div>
              <div className="space-y-2">
                <Label>Fim *</Label>
                <DatePickerField value={formEnd} onChange={setFormEnd} placeholder="Data de fim" />
              </div>
            </div>
