...
Deno.serve(async (req) => {
  ...
  try {
    // Use Lovable Cloud Supabase where push_subscriptions table lives
...
async function createVapidJwt(endpoint: string, publicKey: string, privateKey: string, subject: string) {
  const origin = new URL(endpoint).origin;
  const now = Math.floor(Date.now() / 1000);
  const header = { typ: "JWT", alg: "ES256" };
  const payload = { aud: origin, exp: now + 12 * 60 * 60, sub: subject };
  ...
  const cryptoKey = await crypto.subtle.importKey(
    "jwk",
    { kty: "EC", crv: "P-256", d: privateKey, x: publicKey.slice(0, 43), y: publicKey.slice(43) },
  ...
  return { token: `${unsignedToken}.${sigB64}`, publicKey };
}
...
async function encryptPayload(payload: string, p256dhKey: string, authSecret: string): Promise<Uint8Array> {
  ...
  const subscriberPubKey = await crypto.subtle.importKey("raw", base64UrlToBytes(p256dhKey), { name: "ECDH", namedCurve: "P-256" }, false, []);
  const sharedSecret = await crypto.subtle.deriveBits({ name: "ECDH", public: subscriberPubKey }, localKeyPair.privateKey, 256);
  ...
  const cekBits = (await hmacSha256(prk, concatBytes(cekInfo, new Uint8Array([1])))).slice(0, 16);
  const nonceBits = (await hmacSha256(prk, concatBytes(new TextEncoder().encode("Content-Encoding: nonce\0"), new Uint8Array([1])))).slice(0, 12);
  ...
  const encrypted = await crypto.subtle.encrypt({ name: "AES-GCM", iv: nonceBits }, key, paddedPayload);

  const rs = new Uint8Array(4);
  new DataView(rs.buffer).setUint32(0, 4096);

  return concatBytes(salt, rs, new Uint8Array([65]), localPublicKeyBytes, new Uint8Array(encrypted));
}
